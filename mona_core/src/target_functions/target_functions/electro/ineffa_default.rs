...
use crate::character::characters::electro::ineffa::Ineffa;
...
impl IneffaDefaultTargetFunction {
    pub fn new(config: &TargetFunctionConfig) -> Self {
        match *config {
            TargetFunctionConfig::IneffaDefault {
                recharge_demand,
                use_skill,
                use_burst,
            } => Self {
...
impl TargetFunctionMetaTrait for IneffaDefaultTargetFunction {
    ...
    fn create(_character: &CharacterCommonData, _weapon: &WeaponCommonData, config: &TargetFunctionConfig) -> Box<dyn TargetFunction> {
        Box::new(IneffaDefaultTargetFunction::new(config))
    }
}
...
impl TargetFunction for IneffaDefaultTargetFunction {
    ...
    fn target(&self, attribute: &SimpleAttributeGraph2, character: &Character<SimpleAttributeGraph2>, _weapon: &Weapon<SimpleAttributeGraph2>, _artifacts: &[&Artifact], _enemy: &Enemy) -> f64 {
        ...
        let dmg_normal = Ineffa::damage::<SimpleDamageBuilder>(&context, S::Normal1, &s_config, None).normal.expectation;
        let dmg_skill = Ineffa::damage::<SimpleDamageBuilder>(&context, S::E1, &s_config, None).normal.expectation;
        let dmg_burst = Ineffa::damage::<SimpleDamageBuilder>(&context, S::Q1, &s_config, None).normal.expectation;
