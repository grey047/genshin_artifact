...
use crate::character::characters::anemo::jahoda::Jahoda;
...
pub struct JahodaDefaultTargetFunction {
    pub recharge_demand: f64,
    pub use_skill: f64,
    pub use_burst: f64,
}

impl JahodaDefaultTargetFunction {
    pub fn new(config: &TargetFunctionConfig) -> Self {
        match *config {
            TargetFunctionConfig::JahodaDefault {
                recharge_demand,
                use_skill,
                use_burst,
            } => Self {
...
impl TargetFunctionMetaTrait for JahodaDefaultTargetFunction {
    #[cfg(not(target_family = "wasm"))]
    const META_DATA: TargetFunctionMeta = TargetFunctionMeta {
        name: TargetFunctionName::JahodaDefault,
        ...
        tags: "输出",
        four: TargetFunctionFor::SomeWho(CharacterName::Jahoda),
    ...
    fn create(_character: &CharacterCommonData, _weapon: &WeaponCommonData, config: &TargetFunctionConfig) -> Box<dyn TargetFunction> {
        Box::new(JahodaDefaultTargetFunction::new(config))
    }
}

impl TargetFunction for JahodaDefaultTargetFunction {
    ...
    fn target(&self, attribute: &SimpleAttributeGraph2, character: &Character<SimpleAttributeGraph2>, _weapon: &Weapon<SimpleAttributeGraph2>, _artifacts: &[&Artifact], _enemy: &Enemy) -> f64 {
        ...
        type S = <Jahoda as CharacterTrait>::DamageEnumType;

        let dmg_normal = Jahoda::damage::<SimpleDamageBuilder>(&context, S::Normal1, &s_config, None).normal.expectation;
        let dmg_skill = Jahoda::damage::<SimpleDamageBuilder>(&context, S::E1, &s_config, None).normal.expectation;
        let dmg_burst = Jahoda::damage::<SimpleDamageBuilder>(&context, S::Q1, &s_config, None).normal.expectation;
