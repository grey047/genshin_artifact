...
use crate::character::characters::pyro::durin::Durin;
...
impl DurinDefaultTargetFunction {
    pub fn new(config: &TargetFunctionConfig) -> Self {
        match *config {
            TargetFunctionConfig::DurinDefault {
                recharge_demand,
                use_skill,
                use_burst,
            } => Self {
...
impl TargetFunctionMetaTrait for DurinDefaultTargetFunction {
    ...
    fn create(_character: &CharacterCommonData, _weapon: &WeaponCommonData, config: &TargetFunctionConfig) -> Box<dyn TargetFunction> {
        Box::new(DurinDefaultTargetFunction::new(config))
    }
}
...
impl TargetFunction for DurinDefaultTargetFunction {
    ...
    fn target(&self, attribute: &SimpleAttributeGraph2, character: &Character<SimpleAttributeGraph2>, _weapon: &Weapon<SimpleAttributeGraph2>, _artifacts: &[&Artifact], _enemy: &Enemy) -> f64 {
        ...
        let dmg_normal = Durin::damage::<SimpleDamageBuilder>(&context, S::Normal1, &s_config, None).normal.expectation;
        let dmg_skill = Durin::damage::<SimpleDamageBuilder>(&context, S::E1, &s_config, None).normal.expectation;
        let dmg_burst = Durin::damage::<SimpleDamageBuilder>(&context, S::Q1, &s_config, None).normal.expectation;
