...
use crate::character::characters::dendro::nefer::Nefer;
...
impl NeferDefaultTargetFunction {
    pub fn new(config: &TargetFunctionConfig) -> Self {
        match *config {
            TargetFunctionConfig::NeferDefault {
                recharge_demand,
                use_skill,
                use_burst,
                moonsign_level,
...
impl TargetFunctionMetaTrait for NeferDefaultTargetFunction {
    ...
    fn create(_character: &CharacterCommonData, _weapon: &WeaponCommonData, config: &TargetFunctionConfig) -> Box<dyn TargetFunction> {
        Box::new(NeferDefaultTargetFunction::new(config))
    }
}
...
impl TargetFunction for NeferDefaultTargetFunction {
    ...
    fn target(&self, attribute: &SimpleAttributeGraph2, character: &Character<SimpleAttributeGraph2>, _weapon: &Weapon<SimpleAttributeGraph2>, _artifacts: &[&Artifact], _enemy: &Enemy) -> f64 {
        ...
        type S = <Nefer as CharacterTrait>::DamageEnumType;

        let dmg_normal = Nefer::damage::<SimpleDamageBuilder>(&context, S::Normal1, &s_config, None).normal.expectation;
        let dmg_skill = Nefer::damage::<SimpleDamageBuilder>(&context, S::E1, &s_config, None).normal.expectation;
        let dmg_burst = Nefer::damage::<SimpleDamageBuilder>(&context, S::Q1, &s_config, None).normal.expectation;
